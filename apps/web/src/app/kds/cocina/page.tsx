...
export default function KDSCocinaPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      const res = await authFetch(`/api/kds/cocina`);
      if (res.status === 403) {
        setError("No tiene permisos para acceder a KDS Cocina");
        return;
      }
      if (!res.ok) {
        setConnOk(false);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] p-4">
      <header className="flex items-center justify-between mb-4 md:mb-6 gap-2">
        <div className="flex items-center gap-2 md:gap-4 min-w-0">
          <h1 className="text-xl md:text-2xl font-bold tracking-tight whitespace-nowrap">
            <span className="text-orange-400">KDS</span>{" "}
            <span className="text-white/40 font-light">Cocina</span>
          </h1>
      ...
      {error ? (
        ...
        <div className="flex flex-col items-center justify-center h-[60vh] text-center">
          <div className="text-xl text-white/20">Sin ordenes pendientes</div>
          <div className="text-sm text-white/10 mt-2">
            Las nuevas ordenes aparecen aqui automaticamente
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
